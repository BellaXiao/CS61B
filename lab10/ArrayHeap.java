...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        /** TD: Your code here. */
        // need to consider base case! - index = 1
        int parentIdx = parentIndex(index);
        if (!inBounds(parentIdx)) {
            return;
        }
        int minIdx = min(parentIdx, index);
        if (minIdx == index) {
    ...
    private void sink(int index) {
        ...
        /** TD: Your code here. */
        // consider base case - node with both no left child and right child
        if (!inBounds(leftIndex(index)) && !inBounds(rightIndex(index))) {
            return;
        }

        int minChildIdx = min(leftIndex(index), rightIndex(index));
        int minIdx = min(index, minChildIdx);
        if (minIdx == minChildIdx) {
            swap(index, minChildIdx);
            if (leftIndex(minChildIdx) <= size) {
                sink(minChildIdx);
            }
        }
        return;
    ...
    public T removeMin() {
        /* TD: Your code here! */
        swap(size, 1);
        T res = contents[size].item();
        contents[size] = null;
        size -= 1;
        // corner case again - if the only one of the heap is removed.
        if (size != 0) {
            sink(1);
        }
        return res;
    ...
    public void changePriority(T item, double priority) {
        ...
        int idx = 1;
        while (idx <= size) {
            if (getNode(idx).item().equals(newNode.item())) {
                hasNode = true;
                break;
            }
            idx += 1;
        ...
        if (hasNode) {
            double oldPriority = getNode(idx).priority();
